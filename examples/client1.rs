...
//    let y = vec!(2,4,6);
    ...
    let mut vectors_file = File::open("input.json")
        .expect("Failed to open input.json");

    let mut serialized_vectors = String::new();
    vectors_file.read_to_string(&mut serialized_vectors)
        .expect("Failed to read input.json");

    let (x, y): (Vec<u32>, Vec<u32>) = serde_json::from_str(&serialized_vectors)
        .expect("Failed to deserialize input vectors");
